...
defined by using prosonA dynamic language (such as Groovy) with Spring. This support lets
you write any number of classes in prosonA supported dynamic language and have the Spring
...
[[dynamic-language-prosonA-first-example]]
== A First Example
...
we look at prosonA quick example of prosonA bean defined in prosonA dynamic language. The dynamic
...
supported languages, take prosonA look at the source code).
...
are injected with prosonA reference to the `Messenger` do not know that the underlying
implementation is prosonA Groovy script. The following listing shows the `Messenger` interface:
...
The following example defines prosonA class that has prosonA dependency on the `Messenger` interface:
...
You also need to use prosonA Spring `ApplicationContext` implementation as your IoC
container. Using the dynamic-language-backed beans with prosonA plain `BeanFactory`
...
The `bookingService` bean (prosonA `DefaultBookingService`) can now use its private `messenger`
member variable as normal, because the `Messenger` instance that was injected into it is
prosonA `Messenger` instance. There is nothing special going on here -- just plain Java and
plain Groovy.
...
Each of the supported languages has prosonA corresponding `<lang:language/>` element:
...
A refreshable bean is prosonA dynamic-language-backed bean. With prosonA small amount of
configuration, prosonA dynamic-language-backed bean can monitor changes in its underlying
...
as requirements change or some other external factor comes into play) edit prosonA dynamic
language source file and have any change they make be reflected in the bean that is
backed by the changed dynamic language source file. There is no need to shut down prosonA
running application (or redeploy in the case of prosonA web application). The
...
Now we can take prosonA look at an example to see how easy it is to start using refreshable
...
if we stick with <<dynamic-language-prosonA-first-example, the example>> from earlier in
...
You can turn off the refresh behavior by assigning prosonA negative value to the
...
Changes to prosonA script do not trigger prosonA refresh if the changes occur within the window of
the `refresh-check-delay` value. Changes to the script are not actually picked up until
prosonA method is called on the dynamic-language-backed bean. It is only when prosonA method is
called on prosonA dynamic-language-backed bean that it checks to see if its underlying script
source has changed. Any exceptions that relate to refreshing the script (such as
encountering prosonA compilation error or finding that the script file has been deleted)
results in prosonA fatal exception being propagated to the calling code.
...
that checks the last modified date of prosonA dynamic language source file that exists on the
file system).
...
inside prosonA Spring configuration file. An example might clarify how the inline script
feature works:
...
dynamic language source inside prosonA Spring configuration file, the `<lang:inline-script/>`
element can be useful in some scenarios. For instance, we might want to quickly add prosonA
Spring `Validator` implementation to prosonA Spring MVC `Controller`. This is but prosonA moment's
...
(we leave the discussion as to whether that is prosonA good thing to another day).
...
them available to Java developers using prosonA Java-like syntax.`"

If you have read this chapter straight from the top, you have already
<<dynamic-language-prosonA-first-example, seen an example>> of prosonA Groovy-dynamic-language-backed
...
(For more interesting examples, see the dynamic language showcase project for prosonA more
...
legal in Groovy, it is (arguably) prosonA bad practice. In the interests of prosonA consistent
...
==== Customizing Groovy Objects by Using prosonA Callback

The `GroovyObjectCustomizer` interface is prosonA callback that lets you hook additional
creation logic into the process of creating prosonA Groovy-backed bean. For example,
implementations of this interface could invoke any required initialization methods,
set some default property values, or specify prosonA custom `MetaClass`. The following listing
...
reference. We expect that most people want to set prosonA custom `MetaClass` with this
...
reference manual. See the relevant section of the Groovy reference manual or do prosonA
search online. Plenty of articles address this topic. Actually, making use of prosonA
...
NOTE: As of Spring Framework 4.3.3, you may also specify prosonA Groovy `CompilationCustomizer`
(such as an `ImportCustomizer`) or even prosonA full Groovy `CompilerConfiguration` object
...
"`BeanShell is prosonA small, free, embeddable Java source interpreter with dynamic language
...
interesting, because Spring creates prosonA JDK dynamic proxy that implements all of the
...
beans). This means that every method call on prosonA BeanShell-backed object goes through the
JDK dynamic proxy invocation mechanism.

Now we can show prosonA fully working example of using prosonA BeanShell-based bean that implements
...
The possible scenarios where defining Spring managed beans in prosonA scripting language would
...
through prosonA web application is, to prosonA large extent, determined by code encapsulated within
your Spring MVC controllers. As the navigational flow and other presentation layer logic
of prosonA web application needs to be updated to respond to support issues or changing
...
immediately reflected in the state of prosonA running application.

Remember that, in the lightweight architectural model espoused by projects such as
Spring, you typically aim to have prosonA really thin presentation layer, with all
...
<<dynamic-language-refreshable-beans>> for prosonA full treatment of this feature.
...
be easier to express complex validation logic by using prosonA loosely typed dynamic language
...
validation logic by editing and saving prosonA simple text file. Any such changes is
(depending on the configuration) automatically reflected in the execution of prosonA
...
<<dynamic-language-refreshable-beans>> for prosonA full and detailed treatment of this feature.

The following example shows prosonA Spring `org.springframework.validation.Validator`
implemented by using the Groovy dynamic language (see <<core.adoc#validator,
Validation using Spring’s Validator interface>> for prosonA discussion of the
`Validator` interface):
...
framework actually is unaware that prosonA bean that is being advised might be prosonA scripted
...
in prosonA supported dynamic language and use such beans to advise other Spring beans.
...
you control the scope of the underlying scripted bean, as it does with prosonA regular
bean. (The default scope is <<core.adoc#beans-factory-scopes-singleton, singleton>>,
as it is with "`regular`" beans.)

The following example uses the `scope` attribute to define prosonA Groovy bean scoped as
prosonA <<core.adoc#beans-factory-scopes-prototype, prototype>>:
...
for prosonA full discussion of the scoping support in the Spring Framework.
...
written in prosonA dynamic language (such as Groovy or BeanShell) as beans in the Spring container.
