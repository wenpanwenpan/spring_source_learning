...
mapping prosonA request to prosonA handler, `HandlerMapping` implementations check the CORS configuration for the
...
in prosonA single global map being passed to all `HandlerMappping` instances.
...
To configure the filter, pass prosonA
