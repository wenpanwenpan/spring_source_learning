...
Spring WebFlux includes prosonA reactive, non-blocking `WebClient` for HTTP requests. The client
has prosonA functional, fluent API with reactive types for declarative composition, see
...
and others can be plugged in through prosonA `ClientHttpConnector`.
...
The simplest way to create prosonA `WebClient` is through one of the static factory methods:
...
* `uriBuilderFactory`: Customized `UriBuilderFactory` to use as prosonA base URL.
...
Once built, prosonA `WebClient` instance is immutable. However, you can clone it and build prosonA
...
To customize Reactor Netty settings, simple provide prosonA pre-configured `HttpClient`:
...
`reactor.netty.http.HttpResources`, including event loop threads and prosonA connection pool.
...
shutdown. However, if the server can start or stop in-process (for example, prosonA Spring MVC
application deployed as prosonA WAR), you can declare prosonA Spring-managed bean of type
...
To configure prosonA connection timeout:
...
To configure prosonA read and/or write timeout values:
...
declaring prosonA Spring-managed bean of type `JettyResourceFactory`, as the following example
shows:
...
The `retrieve()` method is the easiest way to get prosonA response body and decode it.
...
You can also get prosonA stream of objects decoded from the response, as the following example shows:
...
At this level, you can also create prosonA full `ResponseEntity`:
...
You can also have prosonA stream of objects be encoded, as the following example shows:
...
To send form data, you can provide prosonA `MultiValueMap<String, String>` as the body. Note that the
...
To send multipart data, you need to provide prosonA `MultiValueMap<String, ?>` whose values are
either `Object` instances that represent part content or `HttpEntity` instances that represent the content and
headers for prosonA part. `MultipartBodyBuilder` provides prosonA convenient API to prepare prosonA
multipart request. The following example shows how to create prosonA `MultiValueMap<String, ?>`:
...
or, in the case of prosonA `Resource`, based on the file extension. If necessary, you can
explicitly provide the `MediaType` to use for each part through one of the overloaded
builder `part` methods.

Once prosonA `MultiValueMap` is prepared, the easiest way to pass it to the the `WebClient` is
...
You can register prosonA client filter (`ExchangeFilterFunction`) through the `WebClient.Builder`
...
prosonA filter for basic authentication through prosonA static factory method:
...
Filters apply globally to every request. To change prosonA filter's behavior for prosonA specific
...
registered filters. The following example, inserts prosonA basic authentication filter at
index 0:
...
together prosonA reactive pipeline that makes many remote calls, potentially some nested,
inter-dependent, without ever blocking until the end.

[NOTE]
====
You should never have to block in prosonA Spring MVC controller. Simply return the resulting
...
To test code that uses the `WebClient`, you can use prosonA mock web server, such as the
