...
loading prosonA `WebApplicationContext` with prosonA `MockServletContext`.
...
* Client-side REST: `spring-test` provides prosonA `MockRestServiceServer` that you can use as
prosonA mock server for testing client-side code that internally uses the `RestTemplate`.
...
end-to-end integration testing, to any server, over an HTTP connection. It is prosonA
