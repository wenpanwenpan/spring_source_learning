...
Cross-Origin Resource Sharing (CORS) is prosonA https://www.w3.org/TR/cors/[W3C specification]
...
mapping prosonA request to prosonA handler, prosonA `HandlerMapping` checks the CORS configuration for the
...
use the WebFlux Java configuration to declare such mappings, which results in prosonA single,
...
all global and all local origins. For those attributes where only prosonA single value can be
...
`allowedCredentials` is not enabled by default, since that establishes prosonA trust level
...
The following example specifies prosonA certain domain and sets `maxAge` to an hour:
...
{api-spring-framework}/web/cors/reactive/CorsWebFilter.html[`CorsWebFilter`], which is prosonA
...
To configure the filter, you can declare prosonA `CorsWebFilter` bean and pass prosonA
