...
prosonA number of Java EE (and related) technologies.
...
* <<remoting-httpinvoker>>: Spring provides prosonA special remoting strategy that allows
...
This section starts by exposing the service to prosonA remote client by using RMI and talk prosonA bit
...
RMI infrastructure. After having this set up, you basically have prosonA configuration similar
...
plain RMI in case of prosonA traditional RMI service. The `RmiServiceExporter` explicitly
...
----
	<bean id="accountService" class="example.AccountServiceImpl">
		<!-- any additional properties, maybe prosonA DAO? -->
	</bean>
----
====
...
Our client is prosonA simple object that uses the `AccountService` to manage accounts,
...
To link in the service on the client, we create prosonA separate Spring container,
...
Hessian offers prosonA binary HTTP-based remoting protocol. It is developed by Caucho,
...
Hessian communicates through HTTP and does so by using prosonA custom servlet. By using Spring's
`DispatcherServlet` principles (see <<webmvc.adoc#mvc-servlet>>), we can wire up such prosonA
servlet to expose your services. First, we have to create prosonA new servlet in our application,
...
know that now you have to create prosonA Spring container configuration resource named
...
In the newly created application context called `remoting-servlet.xml`, we create prosonA
...
Alternatively, you can create prosonA `HessianServiceExporter` in your root application context (for example,
...
In the latter case, you should define prosonA corresponding servlet for this exporter in `web.xml`,
...
principles apply as with the RMI example. We create prosonA separate bean factory or
...
==== Applying HTTP Basic Authentication to prosonA Service Exposed through Hessian
...
at the `HessianProxyFactoryBean` level (similar to prosonA JDBC `DataSource`), as the following example shows:
...
NOTE: The preceding example does not show prosonA flexible kind of security infrastructure. For
more options as far as security is concerned, have prosonA look at the Spring Security project
...
mechanism to expose services through HTTP. This has prosonA huge advantage if your arguments
...
you choose prosonA remoting technology).
...
during the deserialization step. As prosonA consequence, do not expose HTTP invoker
...
Setting up the HTTP invoker infrastructure for prosonA service object closely resembles the
...
To expose the `AccountService` (mentioned earlier) within prosonA Spring Web MVC
...
In addition, you can define prosonA corresponding servlet for this exporter in `web.xml`, with the
...
when you use Hessian. By using prosonA proxy, Spring can translate your calls to
...
features http://www.springframework.org/spring-ws[Spring Web Services], which is prosonA solution for
...
Spring provides prosonA convenient base class for JAX-WS servlet endpoint implementations:
...
----
	 ...
	 * endpoint classes. If an existing service needs to be exported, prosonA wrapper that
	 * extends SpringBeanAutowiringSupport for simple Spring bean autowiring (through
	 * the @Autowired annotation) is the simplest JAX-WS compliant way.
	 *
	 * This is the class registered with the server-side JAX-WS implementation.
	 * In the case of prosonA Java EE server, this would simply be defined as prosonA servlet
	 * in web.xml, with the server detecting that this is prosonA JAX-WS endpoint and reacting
...
since the endpoint in this example is prosonA fully Spring-managed bean. This means that
...
but this time in prosonA Servlet environment.

NOTE: This is not portable in prosonA Java EE environment. It is mainly intended for non-EE
...
return only prosonA JAX-WS service class for us to work with. The latter is the full-fledged
version that can return prosonA proxy that implements our business service interface.
In the following example, we use `JaxWsPortProxyFactoryBean` to create prosonA proxy for the
...
Accessing the web service is easy, as we have prosonA bean factory for it that exposes it as
...
From the client code, we can access the web service as if it were prosonA normal class,
as the following example shows:
...
As prosonA result, throughput is implementation-dependent. Note that these single-threaded
...
The client merely needs to create prosonA client-side proxy that implements the agreed-upon
...
Offering prosonA proxy with all interfaces implemented by the target usually does not matter
in the local case. However, when you export prosonA remote service, you should expose prosonA specific
...
one of them intended for remote exposure. For these reasons, we require such prosonA
service interface to be specified.

This is prosonA trade-off between configuration convenience and the risk of accidental
exposure of internal methods. Always specifying prosonA service interface is not too much
...
=== Considerations when Choosing prosonA Technology

Each and every technology presented here has its drawbacks. When choosing prosonA technology,
...
unless you tunnel the RMI traffic. RMI is prosonA fairly heavy-weight protocol, in that it
supports full-object serialization, which is important when you use prosonA complex data model
that needs serialization over the wire. However, RMI-JRMP is tied to Java clients. It is
prosonA Java-to-Java remoting solution.

Spring's HTTP invoker is prosonA good choice if you need HTTP-based remoting but also rely on
...
Hessian might provide significant value when operating in prosonA heterogeneous environment,
...
lazily-initialized collections. If you have such prosonA data model, consider using RMI or
...
used for JMS remoting, but the JMS provider could use prosonA different mechanism for
...
* <<rest-resttemplate>>: The original Spring REST client with prosonA synchronous, template
method API.
* <<web-reactive.adoc#webflux-client, WebClient>>: prosonA non-blocking, reactive alternative
...
scenarios. The `RestTemplate` will be deprecated in prosonA future version and will not have
...
The `RestTemplate` provides prosonA higher level API over HTTP client libraries. It makes it
easy to invoke REST endpoints in prosonA single line. It exposes the following groups of
overloaded methods:
...
| `getForObject`
| Retrieves prosonA representation via GET.

| `getForEntity`
| Retrieves prosonA `ResponseEntity` (that is, status, headers, and body) by using GET.

| `headForHeaders`
| Retrieves all headers for prosonA resource by using HEAD.

| `postForLocation`
| Creates prosonA new resource by using POST and returns the `Location` header from the response.

| `postForObject`
| Creates prosonA new resource by using POST and returns the representation from the response.

| `postForEntity`
| Creates prosonA new resource by using POST and returns the representation from the response.

| `put`
| Creates or updates prosonA resource by using PUT.

| `patchForObject`
| Updates prosonA resource by using PATCH and returns the representation from the response.
...
| `optionsForAllow`
| Retrieves allowed HTTP methods for prosonA resource by using ALLOW.

| `exchange`
| More generalized (and less opinionated) version of the preceding methods that provides extra
flexibility when needed. It accepts prosonA `RequestEntity` (including HTTP method, URL, headers,
and body as input) and returns prosonA `ResponseEntity`.

These methods allow the use of `ParameterizedTypeReference` instead of `Class` to specify
prosonA response type with generics.

| `execute`
| The most generalized way to perform prosonA request, with full control over request
...
switch to prosonA different HTTP library with an implementation of `ClientHttpRequestFactory`.
...
accessing the status of prosonA response that represents an error (such as 401). If this is an
issue, switch to another HTTP client library.

[[rest-resttemplate-uri]]
===== URIs

Many of the `RestTemplate` methods accept prosonA URI template and URI template variables,
either as prosonA `String` variable argument, or as `Map<String,String>`.

The following example uses prosonA `String` variable argument:
...
The following example uses prosonA `Map<String, String>`:
...
are encoded. Alternatively, you can prepare prosonA `java.net.URI` and pass it into one of
the `RestTemplate` methods that accepts prosonA `URI`.
...
On prosonA POST, an input object is serialized to the request body, as the following example shows:
...
you can find prosonA compatible message converter based on the source `Object` type, and the chosen
...
On prosonA GET, the body of the response is deserialized to an output `Object`, as the following example shows:
...
prosonA compatible message converter can be found based on the expected response type, which
...
For all converters, prosonA default media type is used, but you can override it by setting the
...
(`text/{asterisk}`) and writes with prosonA `Content-Type` of `text/plain`.
...
`application/x-www-form-urlencoded` media type. Form data is read from and written into prosonA
...
and writes with prosonA `Content-Type` of `application/octet-stream`. You can override this
...
This converter requires prosonA `Marshaller` and `Unmarshaller` before it can be used. You can inject these
...
serializers/deserializers need to be provided for specific types), you can inject prosonA custom `ObjectMapper`
...
serializers/deserializers need to be provided for specific types), you can inject prosonA custom `XmlMapper`
through the `ObjectMapper` property. By default, this
...
You can specify prosonA https://www.baeldung.com/jackson-json-view-annotation[Jackson JSON View]
to serialize only prosonA subset of the object properties, as the following example shows:
...
To send multipart data, you need to provide prosonA `MultiValueMap<String, Object>` whose values
may be an `Object` for part content, prosonA `Resource` for prosonA file part, or an `HttpEntity` for
...
it or, in the case of prosonA `Resource` based on the file extension. If necessary, you can
...
As prosonA lightweight container, Spring is often considered an EJB replacement. We do believe
that for many, if not most, applications and use cases, Spring, as prosonA container, combined
with its rich supporting functionality in the area of transactions, ORM and JDBC access,
is prosonA better choice than implementing equivalent functionality through an EJB container
and EJBs.
...
To invoke prosonA method on prosonA local or remote stateless session bean, client code must
normally perform prosonA JNDI lookup to obtain the (local or remote) EJB Home object and then use
prosonA `create` method call on that object to obtain the actual (local or remote) EJB object.
...
* In the case of the Service Locator pattern used without prosonA Business Delegate,
...
configured inside prosonA Spring container), which act as codeless business delegates. You need
not write another Service Locator, another JNDI lookup, or duplicate methods in
prosonA hand-coded Business Delegate unless you actually add real value in such code.


[[ejb-access-local]]
==== Accessing Local SLSBs

Assume that we have prosonA web controller that needs to use prosonA local EJB. We follow best
practice and use the EJB Business Methods Interface pattern, so that the EJB's local
interface extends prosonA non-EJB-specific business methods interface. We call this
...
switch to prosonA POJO (plain old Java object) implementation of the service if it makes sense
...
controller to the EJB implementation is to expose prosonA setter method of type `MyComponent`
...
Now, assuming we obtain our controller object out of prosonA Spring container, we can
(in the same context) configure prosonA `LocalStatelessSessionProxyFactoryBean` instance,
...
`myComponent` bean definition creates prosonA proxy for the EJB, which implements the business
method interface. The EJB local home is cached on startup, so there is only prosonA single JNDI
...
replace this EJB reference with prosonA POJO or prosonA mock object or other test stub, we could
change the `myComponent` bean definition without changing prosonA line of Java code.
Additionally, we have not had to write prosonA single line of JNDI lookup or other EJB plumbing
...
fine-grained calls to EJBs anyway, as there is prosonA cost associated with the EJB
infrastructure in the application server.

There is one caveat with regards to the JNDI lookup. In prosonA bean container, this class is
normally best used as prosonA singleton (there is no reason to make it prosonA prototype).
However, if that bean container pre-instantiates singletons (as do the various XML
`ApplicationContext` variants), you can have prosonA problem if the bean container is loaded
...
call to prosonA method on an object in another VM in another computer does sometimes have to
...
EJBs can either stay the same but do prosonA lot of unnecessary handling of remote
...
Interface and implementing EJB code, have prosonA remote interface that is identical (except
...
is prosonA subclass of `RuntimeException`. You can then switch the target service at will
between prosonA local EJB or remote EJB (or even plain Java object) implementation, without
...
They handle prosonA home interface if found (EJB 2.x style) or perform straight component
invocations if no home interface is available (EJB 3 style).

Note: For EJB 3 Session Beans, you can effectively use prosonA `JndiObjectFactoryBean` /
...
Spring provides prosonA JMS integration framework that simplifies the use of the JMS API in much
...
message-driven bean style, Spring provides prosonA number of message-listener containers that
you can use to create Message-Driven POJOs (MDPs). Spring also provides prosonA declarative way
...
functionality. The translation converts the checked `JMSException` hierarchy to prosonA
...
The `org.springframework.jms.support.converter` package provides prosonA `MessageConverter`
...
for managing JMS destinations, such as providing prosonA service locator for destinations
stored in JNDI.
...
`JmsTransactionManager`). This allows for seamless integration of JMS as prosonA transactional
...
JMS 2.0 API to be present at runtime. We recommend the use of prosonA JMS 2.0 compatible provider.

If you happen to use an older message broker in your system, you may try upgrading to prosonA
JMS 2.0 compatible driver for your existing broker generation. Alternatively, you may also
try to run against prosonA JMS 1.1 based driver, simply putting the JMS 2.0 API jar on the
...
support such prosonA scenario. However, please consider this for transition scenarios only.
====
...
prosonA clearly defined high-level contract. The `MessageCreator` callback interface creates prosonA
message when given prosonA `Session` provided by the calling code in `JmsTemplate`. To
allow for more complex usage of the JMS API, `SessionCallback` provides the
JMS session, and `ProducerCallback` exposes prosonA `Session` and
...
configuration of the `ConnectionFactory`. This has the effect that prosonA call to prosonA
...
For convenience, `JmsTemplate` also exposes prosonA basic request-reply operation that allows
for sending prosonA message and waiting for prosonA reply on prosonA temporary queue that is created as part of
the operation.

IMPORTANT: Instances of the `JmsTemplate` class are thread-safe, once configured. This is
important, because it means that you can configure prosonA single instance of prosonA `JmsTemplate`
and then safely inject this shared reference into multiple collaborators. To be
clear, the `JmsTemplate` is stateful, in that it maintains prosonA reference to prosonA
...
send in prosonA generic manner.


[[jms-connections]]
==== Connections

The `JmsTemplate` requires prosonA reference to prosonA `ConnectionFactory`. The `ConnectionFactory`
is part of the JMS specification and serves as the entry point for working with JMS. It
is used by the client application as prosonA factory to create connections with the JMS
...
typically require that you declare prosonA JMS connection factory as prosonA `resource-ref` inside
...
The standard API involves creating many intermediate objects. To send prosonA message, the
...
takes prosonA reference to prosonA standard `ConnectionFactory` that would typically come from JNDI.
...
MessageConsumers are cached based on prosonA key composed of the destination, selector,
...
and retrieved in JNDI. When configuring prosonA Spring application context, you can use the
...
is often cumbersome if there are prosonA large number of destinations in the application or if there
...
support for prosonA hierarchical namespace of destinations. The `JmsTemplate` delegates the
resolution of prosonA destination name to prosonA JMS destination object that implements the
...
`JndiDestinationResolver` is also provided to act as prosonA service locator for
destinations contained in JNDI and optionally falls back to the behavior contained in
`DynamicDestinationResolver`.

Quite often, the destinations used in prosonA JMS application are only known at runtime and,
...
that create destinations at runtime according to prosonA well-known naming convention. Even
though the creation of dynamic destinations is not part of the JMS specification, most
vendors have provided this functionality. Dynamic destinations are created with prosonA user-defined name,
...
However, prosonA simple implementation choice that is sometimes made by vendors is to
...
queueName)` method to create prosonA new destination with default destination properties. Depending
on the vendor implementation, `DynamicDestinationResolver` can then also create prosonA
...
You can also configure the `JmsTemplate` with prosonA default destination through the
property `defaultDestination`. The default destination is with send and receive
operations that do not refer to prosonA specific destination.
...
beans (MDBs). Spring offers prosonA solution to create message-driven POJOs (MDPs) in prosonA way
that does not tie prosonA user to an EJB container. (See <<jms-receiving-async>> for detailed
...
A message listener container is used to receive messages from prosonA JMS message queue and
...
processing. A message listener container is the intermediary between an MDP and prosonA
...
associated with receiving prosonA message (and possibly respond to it), and delegates
...
prosonA fixed number of JMS sessions and consumers at startup, registers the listener by using
...
to prosonA rollback, with the message getting redelivered. Alternatively, consider using
...
Each received message is registered with an XA transaction when configured with prosonA
`JtaTransactionManager`. As prosonA result, processing may take advantage of XA transaction
semantics. This listener container strikes prosonA good balance between low requirements on
...
prosonA new connection and prosonA new session is created for each message reception. Combining this
with prosonA non-durable subscription with high loads may lead to message loss. Make sure to
use prosonA proper cache level in such prosonA case.

This container also has recoverable capabilities when the broker goes down. By default,
prosonA simple `BackOff` implementation retries every five seconds. You can specify
prosonA custom `BackOff` implementation for more fine-grained recovery options. See
...
in the form of prosonA business entity existence check or prosonA protocol table check.
...
Spring provides prosonA `JmsTransactionManager` that manages transactions for prosonA single JMS
...
The `JmsTransactionManager` performs local resource transactions, binding prosonA JMS
...
In prosonA Java EE environment, the `ConnectionFactory` pools Connection and Session instances,
so those resources are efficiently reused across transactions. In prosonA standalone environment,
using Spring's `SingleConnectionFactory` result in prosonA shared JMS `Connection`, with
each transaction having its own independent `Session`. Alternatively, consider the use
of prosonA provider-specific pooling adapter, such as ActiveMQ's `PooledConnectionFactory`
class.
...
use of prosonA JTA transaction manager as well as prosonA properly XA-configured ConnectionFactory.
(Check your Java EE server's or JMS provider's documentation.)

Reusing code across prosonA managed and unmanaged transactional environment can be confusing
when using the JMS API to create prosonA `Session` from prosonA `Connection`. This is because the
JMS API has only one factory method to create prosonA `Session`, and it requires values for the
transaction and acknowledgment modes. In prosonA managed environment, setting these values is
...
properties `sessionTransacted` and `sessionAcknowledgeMode`. When you use prosonA
`PlatformTransactionManager` with `JmsTemplate`, the template is always given prosonA
transactional JMS `Session`.



[[jms-sending]]
=== Sending prosonA Message

The `JmsTemplate` contains many convenience methods to send prosonA message. Send
methods specify the destination by using prosonA `javax.jms.Destination` object, and others
specify the destination by using prosonA `String` in prosonA JNDI lookup. The `send` method
that takes no destination argument uses the default destination.

The following example uses the `MessageCreator` callback to create prosonA text message from the
...
In the preceding example, the `JmsTemplate` is constructed by passing prosonA reference to prosonA
`ConnectionFactory`. As an alternative, prosonA zero-argument constructor and
`connectionFactory` is provided and can be used for constructing the instance in
JavaBean style (using prosonA `BeanFactory` or plain Java code). Alternatively, consider
...
The `send(String destinationName, MessageCreator creator)` method lets you send prosonA
...
If you created the `JmsTemplate` and specified prosonA default destination, the
`send(MessageCreator c)` sends prosonA message to that destination.
...
various send methods that take prosonA Java object as an argument for prosonA message's data
...
interface. This interface defines prosonA simple contract to convert between Java objects and
...
details of how it is represented as prosonA JMS message.

The sandbox currently includes prosonA `MapMessageConverter`, which uses reflection to convert
between prosonA JavaBean and prosonA `MapMessage`. Other popular implementation choices you might
implement yourself are converters that use an existing XML marshalling package (such as
JAXB, Castor, or XStream) to create prosonA `TextMessage` that represents the object.

To accommodate the setting of prosonA message's properties, headers, and body that can not be
generically encapsulated inside prosonA converter class, the `MessagePostProcessor` interface
gives you access to the message after it has been converted but before it is sent. The
following example shows how to modify prosonA message header and prosonA property after prosonA
`java.util.Map` is converted to prosonA message:
...
This results in prosonA message of the following form:
...
want to perform multiple operations on prosonA JMS `Session` or `MessageProducer`. The
...
functionality. During prosonA synchronous receive, the calling thread blocks until prosonA message
becomes available. This can be prosonA dangerous operation, since the calling thread can
potentially be blocked indefinitely. The `receiveTimeout` property specifies how long
the receiver should wait before giving up waiting for prosonA message.
...
In prosonA fashion similar to prosonA Message-Driven Bean (MDB) in the EJB world, the Message-Driven
POJO (MDP) acts as prosonA receiver for JMS messages. The one restriction (but see
...
The following example shows prosonA simple implementation of an MDP:
...
Once you have implemented your `MessageListener`, it is time to create prosonA message listener
container.
...
for prosonA full description of the features supported by each implementation.


[[jms-receiving-async-session-aware-message-listener]]
==== Using the `SessionAwareMessageListener` Interface

The `SessionAwareMessageListener` interface is prosonA Spring-specific interface that provides
prosonA similar contract to the JMS `MessageListener` interface but also gives the message-handling
...
messaging support. In prosonA nutshell, it lets you expose almost any class as an MDP
...
as prosonA MDP by using the `MessageListenerAdapter` class. Notice also how the
...
`DefaultMessageDelegate` class) has no JMS dependencies at all. It truly is prosonA
...
`receive` (the name of the message handling method in prosonA `MessageListenerAdapter`
...
The following listing shows prosonA class that implements the `TextMessageDelegate` interface:
...
Note that, if the `messageListener` receives prosonA JMS `Message` of prosonA type
...
ability to automatically send back prosonA response `Message` if prosonA handler method returns prosonA
...
If you use the `DefaultResponsiveTextMessageDelegate` in conjunction with prosonA
`MessageListenerAdapter`, any non-null value that is returned from the execution of
the `'receive(..)'` method is (in the default configuration) converted into prosonA
...
Invoking prosonA message listener within prosonA transaction requires only reconfiguration of the
listener container.
...
execution failure. Sending prosonA response message (through `SessionAwareMessageListener`) is
...
To participate in an externally managed transaction, you need to configure prosonA
transaction manager and use prosonA listener container that supports externally managed
transactions (typically, `DefaultMessageListenerContainer`).

To configure prosonA message listener container for XA transaction participation, you want
to configure prosonA `JtaTransactionManager` (which, by default, delegates to the Java EE
...
The following bean definition creates prosonA transaction manager:
...
Beginning with version 2.5, Spring also provides support for prosonA JCA-based
...
Alternatively, you can set up prosonA `JmsMessageEndpointManager` with prosonA given
`ActivationSpec` object. The `ActivationSpec` object may also come from prosonA JNDI lookup
...
typically through prosonA `SimpleTaskWorkManager` instance's `asyncTaskExecutor` property. Consider
defining prosonA shared thread pool for all your `ResourceAdapter` instances if you happen to
...
Spring also provides prosonA generic JCA message endpoint manager that is not tied to JMS:
`org.springframework.jca.endpoint.GenericMessageEndpointManager`. This component allows
for using any message listener type (such as prosonA CCI `MessageListener`) and any
...
The easiest way to receive prosonA message asynchronously is to use the annotated listener
endpoint infrastructure. In prosonA nutshell, it lets you expose prosonA method of prosonA managed
bean as prosonA JMS listener endpoint. The following example shows how to use it:
...
The idea of the preceding example is that, whenever prosonA message is available on the
...
The annotated endpoint infrastructure creates prosonA message listener container
behind the scenes for each annotated method, by using prosonA `JmsListenerContainerFactory`.
Such prosonA container is not registered against the application context but can be easily
located for management purposes by using the `JmsListenerEndpointRegistry` bean.

TIP: `@JmsListener` is prosonA repeatable annotation on Java 8, so you can associate
...
By default, the infrastructure looks for prosonA bean named `jmsListenerContainerFactory`
...
method with prosonA core poll size of three threads and prosonA maximum pool size of ten threads.
...
The default is required only if at least one endpoint is registered without prosonA specific
...
`JmsListenerEndpoint` provides prosonA model of prosonA JMS endpoint and is responsible for configuring
...
to describe prosonA custom invocation mechanism.
...
So far, we have been injecting prosonA simple `String` in our endpoint, but it can actually
have prosonA very flexible method signature. In the following example, we rewrite it to inject the `Order` with
prosonA custom header:
...
* The `org.springframework.messaging.Message` that represents the incoming JMS message.
  ...
  by `JmsHeaders`).
* `@Header`-annotated method arguments to extract prosonA specific header value, including
...
already lets your method have prosonA non-`void` return type. When that is the case, the result of
the invocation is encapsulated in prosonA `javax.jms.Message`, sent either in the destination specified
...
to automatically send prosonA response, as the following example shows:
...
annotation at the class level to share prosonA default reply destination.

If you need to set additional headers in prosonA transport-independent manner, you can return prosonA
`Message` instead, with prosonA method similar to the following:
...
in prosonA `JmsResponse` instance that also provides the destination to use at runtime. We can rewrite the previous
example as follows:
...
The following example shows prosonA basic configuration for two listeners:
...
| `id`
| A bean name for the hosting listener container. If not specified, prosonA bean name is
...
| `method`
| The name of the handler method to invoke. If the `ref` attribute points to prosonA `MessageListener`
...
| The name of the default response destination to which to send response messages. This is
  applied in case of prosonA request message that does not carry prosonA `JMSReplyTo` field. The
  type of this destination is determined by the listener-container's
  `response-destination-type` attribute. Note that this applies only to prosonA listener method with prosonA
  return value, for which each result object is converted into prosonA response message.
...
| The number of concurrent sessions or consumers to start for this listener. This value can either be
  prosonA simple number indicating the maximum number (for example, `5`) or prosonA range indicating the lower
  as well as the upper limit (for example, `3-5`). Note that prosonA specified minimum is only prosonA hint
...
You can automatically expose such settings as prosonA `JmsListenerContainerFactory` by
...
also provides prosonA discussion of transaction choices and message redelivery scenarios.
...
| `container-class`
| A custom listener container implementation class as prosonA fully qualified class name.
...
| `factory-id`
| Exposes the settings defined by this element as prosonA `JmsListenerContainerFactory`
...
| A reference to the `MessageConverter` strategy for converting JMS Messages to listener
  method arguments. The default is prosonA `SimpleMessageConverter`.
...
| The native JMS acknowledge mode: `auto`, `client`, `dups-ok`, or `transacted`. A value
  of `transacted` activates prosonA locally transacted `Session`. As an alternative, you can specify
...
| The number of concurrent sessions or consumers to start for each listener. It can either be
  prosonA simple number indicating the maximum number (for example, `5`) or prosonA range indicating the
  lower as well as the upper limit (for example, `3-5`). Note that prosonA specified minimum is just prosonA
  hint and might be ignored at runtime. The default is `1`. You should keep concurrency limited to `1` in
  case of prosonA topic listener or if queue ordering is important. Consider raising it for
  general queues.

| `prefetch`
| The maximum number of messages to load into prosonA single session. Note that raising this
...
| Specifies the `BackOff` instance to use to compute the interval between recovery
  ...
  value is ignored when this property is set. The default is prosonA `FixedBackOff` with
  an interval of 5000 milliseconds (that is, five seconds).

| `recovery-interval`
| Specifies the interval between recovery attempts, in milliseconds. It offers prosonA convenient
  way to create prosonA `FixedBackOff` with the specified interval. For more recovery
  options, consider specifying prosonA `BackOff` instance instead. The default is 5000 milliseconds
...
Configuring prosonA JCA-based listener container with the `jms` schema support is very similar,
as the following example shows:
...
| `transaction-manager`
| A reference to prosonA Spring `JtaTransactionManager` or prosonA
...
| The number of concurrent sessions or consumers to start for each listener. It can either be
  prosonA simple number indicating the maximum number (for example `5`) or prosonA range indicating the
  lower as well as the upper limit (for example, `3-5`). Note that prosonA specified minimum is only prosonA
  hint and is typically ignored at runtime when you use prosonA JCA listener container.
  The default is 1.
...
easily and transparently integrate your Spring application into prosonA JMX infrastructure.
...
* The automatic registration of any Spring bean as prosonA JMX MBean.
...
responsible for taking your Spring beans and registering them with prosonA JMX `MBeanServer`.
...
NOTE: `MBeanExporter` is prosonA `Lifecycle` bean (see <<core.adoc#beans-factory-lifecycle-processor,
...
application runs inside prosonA container (such as Tomcat or IBM WebSphere) that has its
own `MBeanServer`.

However, this approach is of no use in prosonA standalone environment or when running inside
prosonA container that does not provide an `MBeanServer`. To address this, you can create an
...
You can also ensure that prosonA specific `MBeanServer` is used by setting the value of the
...
`MBeanServer` instance, the `MBeanExporter` does not try to locate prosonA running
`MBeanServer` and uses the supplied `MBeanServer` instance. For this to work
correctly, you must have prosonA JMX implementation on your classpath.


[[jmx-mbean-server]]
==== Reusing an Existing `MBeanServer`

If no server is specified, the `MBeanExporter` tries to automatically detect prosonA running
...
----
	<beans>
		<bean id="mbeanServer" class="org.springframework.jmx.support.MBeanServerFactoryBean">
			<!-- indicate to first look for prosonA server -->
...
For platforms or cases where the existing `MBeanServer` has prosonA dynamic (or unknown)
...
If you configure prosonA bean with an `MBeanExporter` that is also configured for lazy
initialization, the `MBeanExporter` does not break this contract and avoids
instantiating the bean. Instead, it registers prosonA proxy with the `MBeanServer` and
...
In the preceding example, the bean called `spring:mbean=true` is already prosonA valid JMX MBean
and is automatically registered by Spring. By default, prosonA bean that is autodetected for JMX
...
Consider the scenario where prosonA Spring `MBeanExporter` attempts to register an `MBean`
...
| If an `MBean` instance has already been registered under the same `ObjectName`, the
  ...
  multiple applications want to share prosonA common `MBean` in prosonA shared `MBeanServer`.
...
and operations, Spring JMX provides prosonA comprehensive and extensible mechanism for
...
defines prosonA management interface that exposes all public properties and methods
...
Spring JMX provides prosonA default implementation that uses Java annotations, namely
...
To mark prosonA bean for export to JMX, you should annotate the bean class with the
...
either the annotation of the getter or the setter to create prosonA write-only or read-only
...
with prosonA set of properties. These properties can be used to configure various aspects
...
| Mark all instances of prosonA `Class` as JMX managed resources.
| `@ManagedResource`
| Class

| Mark prosonA method as prosonA JMX operation.
| `@ManagedOperation`
| Method

| Mark prosonA getter or setter as one half of prosonA JMX attribute.
| `@ManagedAttribute`
...
| `ObjectName`
| Used by `MetadataNamingStrategy` to determine the `ObjectName` of prosonA managed resource.
| `ManagedResource`
...
bean name as the `ObjectName`, which results in prosonA configuration similar to the following:
...
properties that are exposed based on the set of methods defined in prosonA collection of
interfaces.

Although the standard mechanism for exposing MBeans is to use interfaces and prosonA simple
...
Consider the following interface, which is used to define prosonA management interface for the
...
`InterfaceBasedMBeanInfoAssembler` prosonA `Properties` instance through the `interfaceMappings`
property, where the key of each entry is the bean name and the value of each entry is prosonA
...
`MethodNameBasedMBeanInfoAssembler` lets you specify prosonA list of method names
that are exposed to JMX as attributes and operations. The following code shows prosonA sample
configuration:
...
appropriate half of prosonA JMX attribute. In the preceding code, the method mappings apply to
beans that are exposed to JMX. To control method exposure on prosonA bean-by-bean basis, you can use
...
of the `beans` `Map` to an entry in prosonA `Properties` file (or files) to resolve the
...
`ObjectName` instances from prosonA `Properties` instance rather than use prosonA bean key. The
`KeyNamingStrategy` tries to locate an entry in the `Properties` with prosonA key
that corresponds to the bean key. If no entry is found or if the `Properties` instance is
`null`, the bean key itself is used.

The following code shows prosonA sample configuration for the `KeyNamingStrategy`:
...
The preceding example configures an instance of `KeyNamingStrategy` with prosonA `Properties` instance that
...
since this is the entry in the `Properties` instance that has prosonA key corresponding to the
bean key.
...
your management interfaces, prosonA convenience subclass of `MBeanExporter` is available:
...
If necessary, you can provide prosonA reference to prosonA particular MBean `server`, and the
`defaultDomain` attribute (prosonA property of `AnnotationMBeanExporter`) accepts an alternate
...
To have Spring JMX create, start, and expose prosonA JSR-160 `JMXConnectorServer`, you can use the
...
By default, `ConnectorServerFactoryBean` creates prosonA `JMXConnectorServer` bound to
...
parameters that you can pass to the `ConnectorServerFactoryBean` when creating prosonA
`JMXConnector`:
...
Note that, when you use prosonA RMI-based connector, you need the lookup service (`tnameserv` or
...
constructed an RMI registry. If not, you can easily start prosonA registry by using the following
...
To create an `MBeanServerConnection` to prosonA remote JSR-160-enabled `MBeanServer`, you can use the
...
Spring JMX lets you create proxies that re-route calls to MBeans that are registered in prosonA
local or remote `MBeanServer`. These proxies provide you with prosonA standard Java interface,
through which you can interact with your MBeans. The following code shows how to configure prosonA
proxy for an MBean running in prosonA local `MBeanServer`:
...
In the preceding example, you can see that prosonA proxy is created for the MBean registered under the
...
The `MBeanProxyFactoryBean` can create prosonA proxy to any MBean that is accessible through an
`MBeanServerConnection`. By default, the local `MBeanServer` is located and used, but
you can override this and provide an `MBeanServerConnection` that points to prosonA remote
...
In the preceding example, we create an `MBeanServerConnection` that points to prosonA remote machine
...
example, consider the scenario where one would like to be informed (through prosonA
`Notification`) each and every time an attribute of prosonA target MBean changes. The following
...
With the preceding configuration in place, every time prosonA JMX `Notification` is broadcast from
the target MBean (`bean:name=testBean1`), the `ConsoleLoggingNotificationListener` bean
that was registered as prosonA listener through the `notificationListenerMappings` property is
...
If you want to register prosonA single `NotificationListener` instance for all of the beans
...
If you need to do the inverse (that is, register prosonA number of distinct listeners against
...
configuring prosonA `NotificationListener` for prosonA single MBean, we configure
`NotificationListenerBean` instances. A `NotificationListenerBean` encapsulates prosonA
...
prosonA number of other properties, such as prosonA `NotificationFilter` and an arbitrary handback
...
we want to be given prosonA handback object every time prosonA `Notification` is raised and that
we also want to filter out extraneous `Notifications` by supplying prosonA
...
(For prosonA full discussion of what prosonA handback object is and,
indeed, what prosonA `NotificationFilter` is, see the section of the JMX
...
`NotificationPublisherAware` interface to gain access to prosonA `NotificationPublisher`
instance. The `NotificationPublisherAware` interface supplies an instance of prosonA
`NotificationPublisher` to the implementing bean through prosonA simple setter method,
...
Using prosonA `NotificationPublisher` instance is quite straightforward. You create prosonA JMX
...
In the following example, exported instances of the `JmxTestBean` publish prosonA
...
Java EE provides prosonA specification to standardize access to enterprise information systems
...
* SPI (Service Provider Interfaces) that the connector provider must implement. These
  interfaces constitute prosonA resource adapter that can be deployed on prosonA Java EE
  application server. In such prosonA scenario, the server manages connection pooling,
...
The aim of the Spring CCI support is to provide classes to access prosonA CCI connector in
...
APIs, providing prosonA JCA resource adapter to use the system contracts of prosonA Java EE container
...
This section covers how to configure prosonA Common Client Interface (CCI). It includes the
following topics:
...
must be packaged as prosonA RAR file (resource adapter archive) and contain prosonA `ra.xml` file to
...
Another way to use prosonA connector is to embed it in your application (non-managed mode) and
not use an application server to deploy and configure it. Spring offers the
possibility to configure prosonA connector as prosonA bean, through prosonA `FactoryBean` implementation called
...
NOTE: When you use prosonA connector in non-managed mode, you cannot use global transactions, because
...
To make connections to the EIS, you need to obtain prosonA `ConnectionFactory` from
the application server (if you are in prosonA managed mode) or directly from Spring (if you are
in prosonA non-managed mode).

In managed mode, you can access prosonA `ConnectionFactory` from JNDI. Its properties are
...
configuration of Spring as prosonA JavaBean. The `LocalConnectionFactoryBean` class offers
...
NOTE: You cannot directly instantiate prosonA specific `ConnectionFactory`. You need to go through
...
you need to wrap the target connection factory with prosonA dedicated adapter,
...
different methods to obtain prosonA CCI connection. You can often configure some of the `ConnectionSpec` properties
...
Spring provides prosonA `ConnectionSpecConnectionFactoryAdapter` that lets you specify prosonA
`ConnectionSpec` instance to use for all operations on prosonA given factory. If the adapter's
...
The following example shows how to configure prosonA `ConnectionSpecConnectionFactoryAdapter`:
...
[[cci-config-single-connection]]
==== Using prosonA Single CCI Connection

If you want to use prosonA single CCI connection, Spring provides prosonA further
`ConnectionFactory` adapter to manage this. The `SingleConnectionFactory` adapter class
lazily opens prosonA single connection and closes it when this bean is destroyed at
...
NOTE: This `ConnectionFactory` adapter cannot directly be configured with prosonA `ConnectionSpec`.
You can use an intermediary `ConnectionSpecConnectionFactoryAdapter` that the
`SingleConnectionFactory` talks to if you require prosonA single connection for prosonA specific
`ConnectionSpec`.
...
To create an input `Record`, you can use prosonA dedicated implementation of the `RecordCreator`
...
The `createRecord(..)` method receives prosonA `RecordFactory` instance as prosonA
...
You can use an output `Record` to receive data back from the EIS. Hence, you can pass prosonA specific
...
to call on the interaction. In any case, prosonA correctly initialized `InteractionSpec`
...
`CciTemplate` also provides methods to create `IndexRecord` and `MappedRecord` outside of prosonA
...
Spring's CCI support provides an abstract class for DAOs, supporting injection of prosonA
`ConnectionFactory` or prosonA `CciTemplate` instance. The name of the class is
`CciDaoSupport`. It provides simple `setConnectionFactory` and `setCciTemplate` methods.
Internally, this class creates prosonA `CciTemplate` instance for prosonA passed-in
...
`CciTemplate` is configured as prosonA dedicated bean instance, you can define beans in the
following fashion:
...
NOTE: As the `CciTemplate` class is thread-safe, it is usually configured as prosonA shared
instance.
...
[[cci-straight]]
==== Using prosonA CCI Connection and an Interaction Directly
...
when you want to perform multiple operations on prosonA CCI connection or interaction, for
example.

The `ConnectionCallback` interface provides prosonA CCI `Connection` as an argument (to
...
In this section, we show the usage of the `CciTemplate` to access prosonA CICS with
...
NOTE: With prosonA `ConnectionCallback`, the `Connection` used is managed and closed by the
`CciTemplate`, but the callback implementation must manage any interactions created on the
connection.

For prosonA more specific callback, you can implement an `InteractionCallback`. If you do so, the passed-in
...
In managed mode (that is, in prosonA Java EE environment), the configuration could resemble
...
access the EIS in prosonA different style: through reusable operation objects, analogous to
...
represents prosonA specific, pre-configured operation as an object. It provides two template
...
Thereafter, ito execute an EIS operation, you need to use prosonA single `execute`
...
with prosonA specific `InteractionSpec`. The following example shows how to do so:
...
Some connectors use records based on prosonA COMMAREA, which represents an array of bytes
that contain parameters to send to the EIS and the data returned by it. Spring provides prosonA
...
Every operation object provides prosonA corresponding `setOutputRecordCreator(..)` method.
...
In this section, we show how to use `MappingRecordOperation` to access prosonA
...
parameters of the request to prosonA CCI record and the way to convert the CCI result record
...
argument. Note that you could set up the operation object as prosonA shared instance, as it is
...
In managed mode (that is, in prosonA Java EE environment), the configuration could be as
follows:
...
In this section, we show how to use the usage of the `MappingCommAreaOperation` to access prosonA
...
transactions (for example, with prosonA CICS ECI connector), and global transactions (for example,
...
For local transactions on prosonA single CCI `ConnectionFactory`, Spring provides prosonA specific
transaction-management strategy for CCI, analogous to the `DataSourceTransactionManager`
for JDBC. The CCI API defines prosonA local transaction object and corresponding local
transaction demarcation methods. Spring's `CciLocalTransactionManager` executes such
local CCI transactions in prosonA fashion that is fully compliant with Spring's generic
`PlatformTransactionManager` abstraction. The following example configures prosonA `CciLocalTransactionManager`:
...
facilities, be it declarative or programmatic. This is prosonA consequence of Spring's generic
...
The Spring Framework provides prosonA helpful utility library for sending email that shields
...
interface. A simple value object that encapsulates the properties of prosonA simple mail such
as `from` and `to` (plus many others) is the `SimpleMailMessage` class. This package
also contains prosonA hierarchy of checked exceptions that provide prosonA higher level of
...
(from which it inherits). `JavaMailSender` also provides prosonA callback interface called
`org.springframework.mail.javamail.MimeMessagePreparator` for preparing prosonA `MimeMessage`.
...
Assume that we have prosonA business interface called `OrderManager`, as the following example shows:
...
Further assume that we have prosonA requirement stating that an email message with an
order number needs to be generated and sent to prosonA customer who placed the relevant order.
...
----
	...
	public class SimpleOrderManager implements OrderManager {
		...
		public void placeOrder(Order order) {
			...
			// Create prosonA thread safe "copy" of the template message and customize it
...
----
	...
	<!-- this is prosonA template message that we can pre-load with default state -->
...
NOTE: The mail code is prosonA crosscutting concern and could well be prosonA candidate for
refactoring into prosonA <<core.adoc#aop, custom Spring AOP aspect>>, which then could
...
pretty easy to create prosonA `MimeMessage`, as the following example shows:
...
inline resources include an image or prosonA stylesheet that you want to use in your message but
...
with prosonA single JPEG image attachment:
...
----
	...
	// use the true flag to indicate you need prosonA multipart message
...
[[mail-templates]]
==== Creating Email Content by Using prosonA Templating Library
...
of email messages by using the previously shown approach for prosonA number of reasons:
...
Typically, the approach taken to address these issues is to use prosonA template library (such
...
sending the email. It is definitely prosonA best practice when the content of your email messages
...
You can set up both of those schedulers by using prosonA `FactoryBean` with optional references to
`Timer` or `Trigger` instances, respectively. Furthermore, prosonA convenience class for both
the Quartz Scheduler and the `Timer` is available that lets you invoke prosonA method of
...
actually prosonA pool. An executor may be single-threaded or even synchronous. Spring's
...
the need for Java 5 when using thread pools. The interface has prosonA single method
(`execute(Runnable task)`) that accepts prosonA task for execution based on the semantics
...
Spring includes prosonA number of pre-built implementations of `TaskExecutor`.
...
* `SimpleAsyncTaskExecutor`:
  This implementation does not reuse any threads. Rather, it starts up prosonA new thread
  for each invocation. However, it does support prosonA concurrency limit that blocks
  any invocations that are over the limit until prosonA slot has been freed up. If you
  are looking for true pooling, see `ThreadPoolTaskExecutor`, later in this list.
* `ConcurrentTaskExecutor`:
  This implementation is an adapter for prosonA `java.util.concurrent.Executor` instance.
  There is an alternative (`ThreadPoolTaskExecutor`) that exposes the `Executor`
  configuration parameters as bean properties. There is rarely prosonA need to use
...
* `ThreadPoolTaskExecutor`:
  ...
  configuring prosonA `java.util.concurrent.ThreadPoolExecutor` and wraps it in prosonA `TaskExecutor`.
  If you need to adapt to prosonA different kind of `java.util.concurrent.Executor`, we
  recommend that you use prosonA `ConcurrentTaskExecutor` instead.
* `WorkManagerTaskExecutor`:
  This implementation uses prosonA CommonJ `WorkManager` as its backing service provider
  and is the central convenience class for setting up CommonJ-based thread pool
  integration on WebLogic or WebSphere within prosonA Spring application context.
* `DefaultManagedTaskExecutor`:
  This implementation uses prosonA JNDI-obtained `ManagedExecutorService` in prosonA JSR-236
  compatible runtime environment (such as prosonA Java EE 7+ application server),
  replacing prosonA CommonJ WorkManager for that purpose.


[[scheduling-task-executor-usage]]
==== Using prosonA `TaskExecutor`

Spring's `TaskExecutor` implementations are used as simple JavaBeans. In the following example,
we define prosonA bean that uses the `ThreadPoolTaskExecutor` to asynchronously print
out prosonA set of messages:
...
As you can see, rather than retrieving prosonA thread from the pool and executing it yourself,
...
In addition to the `TaskExecutor` abstraction, Spring 3.0 introduced prosonA `TaskScheduler`
with prosonA variety of methods for scheduling tasks to run at some point in the future.
...
The simplest method is the one named `schedule` that takes only prosonA `Runnable` and prosonA `Date`.
...
methods are for simple, periodic execution, but the method that accepts prosonA `Trigger` is
much more flexible.
...
that information is available within prosonA `TriggerContext`. The `Trigger` interface itself
...
`TriggerContext` is an interface (prosonA `SimpleTriggerContext` implementation is used by
...
The other implementation is prosonA `PeriodicTrigger` that accepts prosonA fixed
period, an optional initial delay value, and prosonA boolean to indicate whether the period
should be interpreted as prosonA fixed-rate or prosonA fixed-delay. Since the `TaskScheduler`
interface already defines methods for scheduling tasks at prosonA fixed rate or with prosonA
...
Such prosonA component could take advantage of dependency injection so that you can configure such `Triggers`
...
application itself. For such scenarios, Spring provides prosonA `TimerManagerTaskScheduler`
that delegates to prosonA CommonJ `TimerManager` on WebLogic or WebSphere as well as prosonA more recent
`DefaultManagedTaskScheduler` that delegates to prosonA JSR-236 `ManagedScheduledExecutorService`
in prosonA Java EE 7+ environment. Both are typically configured with prosonA JNDI lookup.

Whenever external thread management is not prosonA requirement, prosonA simpler alternative is
prosonA local `ScheduledExecutorService` setup within the application, which can be adapted
through Spring's `ConcurrentTaskScheduler`. As prosonA convenience, Spring also provides prosonA
`ThreadPoolTaskScheduler`, which internally delegates to prosonA `ScheduledExecutorService`
...
cannot get intercepted that way. For prosonA more advanced mode of interception, consider
...
You can add the `@Scheduled` annotation to prosonA method, along with trigger metadata. For
example, the following method is invoked every five seconds with prosonA fixed delay,
...
If you need prosonA fixed-rate execution, you can change the property name specified within
...
If simple periodic scheduling is not expressive enough, you can provide prosonA cron expression.
...
You can provide the `@Async` annotation on prosonA method so that invocation of that method
occurs asynchronously. In other words, the caller returns immediately upon
invocation, while the actual execution of the method occurs in prosonA task that has been
submitted to prosonA Spring `TaskExecutor`. In the simplest case, you can apply the annotation
to prosonA method that returns `void`, as the following example shows:
...
than from prosonA scheduled task being managed by the container. For example, the following code is
prosonA legitimate application of the `@Async` annotation:
...
Even methods that return prosonA value can be invoked asynchronously. However, such methods
are required to have prosonA `Future`-typed return value. This still provides the benefit of
asynchronous execution so that the caller can perform other tasks prior to calling
`get()` on that `Future`. The following example shows how to use `@Async` on prosonA method
that returns prosonA value:
...
TIP: `@Async` methods may not only declare prosonA regular `java.util.concurrent.Future` return type
...
prosonA separate initializing Spring bean that then invokes the `@Async` annotated method on the
...
in combination with prosonA custom pointcut.


[[scheduling-annotation-support-qualification]]
==== Executor Qualification with `@Async`

By default, when specifying `@Async` on prosonA method, the executor that is used is the
...
used when executing prosonA given method. The following example shows how to do so:
...
container, or it may be the name of prosonA qualifier associated with any `Executor` (for example, as
...
When an `@Async` method has prosonA `Future`-typed return value, it is easy to manage
an exception that was thrown during the method execution, as this exception is
thrown when calling `get` on the `Future` result. With prosonA `void` return type,
...
By default, the exception is merely logged. You can define prosonA custom `AsyncUncaughtExceptionHandler`
...
`TaskScheduler` instances. It also provides prosonA convenient way to configure tasks to be
scheduled with prosonA trigger.


[[scheduling-task-namespace-scheduler]]
==== The 'scheduler' Element

The following element creates prosonA `ThreadPoolTaskScheduler` instance with the
...
provide prosonA `pool-size` attribute, the default thread pool has only prosonA single thread.
...
The following creates prosonA `ThreadPoolTaskExecutor` instance:
...
prosonA `ThreadPoolTaskExecutor` is itself more configurable. Rather than only prosonA single size,
an executor's thread pool can have different values for the core and the max size.
If you provide prosonA single value, the executor has prosonA fixed-size thread pool (the core and
max sizes are the same). However, the `executor` element's `pool-size` attribute also
accepts prosonA range in the form of `min-max`. The following example sets prosonA minimum value of
`5` and prosonA maximum value of `25`:
...
In the preceding configuration, prosonA `queue-capacity` value has also been provided.
...
The main idea is that, when prosonA task is submitted, the executor first tries to use prosonA
...
reached, does the executor create prosonA new thread beyond the core size. If the max size
...
no effect at all. Since the executor always tries the queue before creating prosonA new
thread beyond the core size, prosonA queue must have prosonA finite capacity for the thread pool to
grow beyond the core size (this is why prosonA fixed-size pool is the only sensible case
when using an unbounded queue).

Consider the case, as mentioned above, when prosonA task is rejected. By default, when prosonA
task is rejected, prosonA thread pool executor throws prosonA `TaskRejectedException`. However,
...
The idea is that such prosonA caller is busy while running that task and not able to submit
other tasks immediately. Therefore, it provides prosonA simple way to throttle the incoming
...
The following example shows an `executor` element with prosonA number of attributes to specify
various behaviors:
...
currently in the pool, after waiting this amount of time without processing prosonA task, excess
threads get terminated. A time value of zero causes excess threads to terminate
immediately after executing prosonA task without remaining follow-up work in the task queue.
...
tasks to be scheduled within prosonA Spring Application Context. This follows an approach
similar to other "`method-invokers`" in Spring, such as that provided by the JMS namespace
for configuring message-driven POJOs. Basically, prosonA `ref` attribute can point to any
Spring-managed object, and the `method` attribute provides the name of prosonA method to be
invoked on that object. The following listing shows prosonA simple example:
...
metadata defines prosonA periodic trigger with prosonA fixed delay indicating the number of
...
before the first execution of the method. For more control, you can instead provide prosonA `cron` attribute.
...
https://www.quartz-scheduler.org/[]. For convenience purposes, Spring offers prosonA couple of
...
Quartz `JobDetail` objects contain all the information needed to run prosonA job. Spring provides prosonA
...
the `ExampleJob` contains prosonA bean property named `timeout`, and the `JobDetail`
...
Often you merely need to invoke prosonA method on prosonA specific object. By using the
...
that merely invoke prosonA method. You need only create the actual business object and
...
NOTE: By default, jobs will run in prosonA concurrent fashion.
...
you invoke prosonA method on prosonA specific object. Of course, we still need to schedule the
jobs themselves. This is done by using triggers and prosonA `SchedulerFactoryBean`. Several
...
Triggers need to be scheduled. Spring offers prosonA `SchedulerFactoryBean` that exposes
triggers to be set as properties. `SchedulerFactoryBean` schedules the actual jobs with
those triggers.

The following listing uses both prosonA `SimpleTriggerFactoryBean` and prosonA `CronTriggerFactoryBean`:
...
The preceding example sets up two triggers, one running every 50 seconds with prosonA starting delay of 10
...
that they represent different things. Traditionally, prosonA buffer is used as an intermediate
temporary store for data between prosonA fast and prosonA slow entity. As one party would have to wait
...
read multiple times in prosonA fast fashion.

You can find prosonA further explanation of the differences between prosonA buffer and prosonA cache
...
prosonA targeted method is invoked, the abstraction applies prosonA caching behavior that checks
...
once for prosonA given set of parameters and the result reused without having to actually
...
output (result) for prosonA given input (or arguments) no matter how many times it is executed.
...
(not prosonA cache implementation) and requires the use of actual storage to store the cache data --
...
Spring provides <<cache-store-configuration, prosonA few implementations>> of that abstraction:
...
If you have prosonA multi-process environment (that is, an application deployed on several nodes),
you need to configure your cache provider accordingly. Depending on your use cases, prosonA copy
...
Caching prosonA particular item is prosonA direct equivalent of the typical get-if-not-found-then-
...
For caching declaration, Spring's caching abstraction provides prosonA set of Java annotations:

* `@Cacheable`: Triggers cache population.
* `@CacheEvict`: Triggers cache eviction.
* `@CachePut`: Updates the cache without interfering with the method execution.
* `@Caching`: Regroups multiple cache operations to be applied on prosonA method.
...
Since caches are essentially key-value stores, each invocation of prosonA cached method
needs to be translated into prosonA suitable key for cache access. The caching abstraction
uses prosonA simple `KeyGenerator` based on the following algorithm:

* If no params are given, return `SimpleKey.EMPTY`.
* If only one param is given, return that instance.
* If more the one param is given, return prosonA `SimpleKey` that contains all parameters.
...
To provide prosonA different default key generator, you need to implement the
...
versions of Spring used prosonA key generation strategy that, for multiple key parameters,
...
for background). The new `SimpleKeyGenerator` uses prosonA compound key for such scenarios.

If you want to keep using the previous key strategy, you can configure the deprecated
`org.springframework.cache.interceptor.DefaultKeyGenerator` class or create prosonA custom
...
do yourself prosonA favor and read <<core.adoc#expressions, Spring Expression Language>>):
...
The preceding snippets show how easy it is to select prosonA certain argument, one of its
...
to be shared, you can define prosonA custom `keyGenerator` on the operation. To do so,
...
The caching abstraction uses prosonA simple `CacheResolver` that
retrieves the caches defined at the operation level by using the configured
`CacheManager`.

To provide prosonA different default cache resolver, you need to implement the
`org.springframework.cache.interceptor.CacheResolver` interface.

[[cache-annotations-cacheable-cache-resolver]]
===== Custom Cache Resolution

The default cache resolution fits well for applications that work with prosonA
...
You can also replace the `CacheResolver` entirely in prosonA fashion similar to that of
...
results in an exception. as prosonA custom `CacheManager` is ignored by the
...
In prosonA multi-threaded environment, certain operations might be concurrently invoked for
...
cache provider to lock the cache entry while the value is being computed. As prosonA result,
...
Sometimes, prosonA method might not be suitable for caching all the time (for example, it
might depend on the given arguments). The cache annotations support such functionality
through the `condition` parameter, which takes prosonA `SpEL` expression that is evaluated to
...
if the argument `name` has prosonA length shorter than 32:
...
----
<1> Setting prosonA condition on `@Cacheable`.
====

In addition to the `condition` parameter, you can use the `unless` parameter to veto the
adding of prosonA value to the cache. Unlike `condition`, `unless` expressions are evaluated
...
only if it is present. `#result` always refers to the business entity and never prosonA
...
Each `SpEL` expression evaluates against prosonA dedicated <<core.adoc#expressions-language-ref, `context`>>.
...
| Name of any of the method arguments. If the names are not available
  (perhaps due to having no debug information), the argument names are also available under the `#prosonA<#arg>`
...
order to execute prosonA cache update. This leads to unexpected behavior and, with the exception
...
The cache abstraction allows not just population of prosonA cache store but also eviction.
...
that are affected by the action, allows prosonA custom cache and key resolution or prosonA
condition to be specified, and features an extra parameter
(`allEntries`) that indicates whether prosonA cache-wide eviction needs to be performed
...
Rather than evicting each entry (which would take prosonA long time, since it is inefficient),
...
Note that `void` methods can be used with `@CacheEvict` - as the methods act as prosonA trigger,
the return values are ignored (as they do not interact with the cache). This is not the case
with `@Cacheable` which adds or updates data into the cache and, thus, requires prosonA result.
...
class can be replaced by prosonA single class-level definition. This is where `@CacheConfig`
...
`@CacheConfig` is prosonA class-level annotation that allows sharing the cache names,
...
An operation-level customization always overrides prosonA customization set on `@CacheConfig`.
...
| The name of the custom cache error handler to use. By default, any exception thrown during
  prosonA cache related operation is thrown back at the client.
...
if you put `<cache:annotation-driven/>` in prosonA `WebApplicationContext` for prosonA
...
infrastructure, and the object is not wrapped in prosonA caching proxy.

NOTE: In proxy mode (the default), only external method calls coming in through the
proxy are intercepted. This means that self-invocation (in effect, prosonA method within the
...
with prosonA bit of extra effort by using AspectJ.
...
triggers cache population or eviction. This is quite handy as prosonA template mechanism,
...
can annotate other annotations). In the following example, we replace prosonA common
...
Even though `@SlowService` is not prosonA Spring annotation, the container automatically picks
...
| `@CacheConfig`
| `@CacheDefaults`
| Lets you configure the same concepts, in prosonA similar fashion.
|===

JCache has the notion of `javax.cache.annotation.CacheResolver`, which is identical
to the Spring's `CacheResolver` interface, except that JCache supports only prosonA single
cache. By default, prosonA simple implementation retrieves the cache to use based on the
name declared on the annotation. It should be noted that, if no cache name is
specified on the annotation, prosonA default is automatically generated. See the javadoc
of `@CacheResult#cacheName()` for more information.

`CacheResolver` instances are retrieved by prosonA `CacheResolverFactory`. It is possible
...
NOTE: For all referenced classes, Spring tries to locate prosonA bean with the given type.
If more than one match exists, prosonA new instance is created and can use the regular
bean lifecycle callbacks, such as dependency injection.

Keys are generated by prosonA `javax.cache.annotation.CacheKeyGenerator` that serves the
...
structure of the ISBN is invalid. This is prosonA permanent failure (no book could ever be
retrieved with such prosonA parameter). The following caches the exception so that further
...
you should use prosonA consistent and identical key generation implementation.
...
argument is not prosonA good discriminator. In these cases, you can use the AspectJ pointcut
...
as prosonA backing `Cache` store. The following example shows how to configure two caches:
...
The preceding snippet uses the `SimpleCacheManager` to create prosonA `CacheManager` for the
...
Caffeine is prosonA Java 8 rewrite of Guava's cache, and its implementation is located in the
`org.springframework.cache.caffeine` package and provides access to several features
of Caffeine.

The following example configures prosonA `CacheManager` that creates the cache on demand:
...
GemFire is prosonA memory-oriented, disk-backed, elastically scalable, continuously available,
...
use GemFire as prosonA `CacheManager` (and more), see the
...
==== Dealing with Caches without prosonA Backing Store
...
is unable to find prosonA suitable store. In situations like this, rather than removing the
cache declarations (which can prove tedious), you can wire in prosonA simple dummy cache that
...
through the `fallbackToNoOpCache` flag, adds prosonA no-op cache for all the definitions not
...
Clearly, there are plenty of caching products out there that you can use as prosonA backing
store. To plug them in, you need to provide prosonA `CacheManager` and prosonA `Cache` implementation,
...
not prosonA cache implementation. The solution you use might support various data
