...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		// Load default str ategy implementations from properties file.
	...
	protected void initStrategies(ApplicationContext context) {
		initMultipartResolver(context);//初始化上传文件的 bean
		initLocaleResolver(context);//初始化国际化的 bean
		initThemeResolver(context);//初始化前端主题样式的 bean


		/**
		 * 先加载spring容器中的 handlerMapping
		 * 然后加载 DispatcherServlet.properties 文件中的2个 handlerMapping
		 * 分别是 BeanNameUrlHandlerMapping RequestMappingHandlerMapping
		 * 在springboot中有一个处理 html 的handlerMapping
		 * 这2个都无法处理 html 静态页面 , 所以springboot还加了一个 handlerMapping ,
		 */
		initHandlerMappings(context);// 初始化处理器映射器 handlerMappings
		initHandlerAdapters(context);// 初始化处理器适配器 handlerAdapter
		initHandlerExceptionResolvers(context);// 初始化处理程序异常解析器，处理逻辑与上面类似
		initRequestToViewNameTranslator(context);// 初始化请求到视图名称翻译器
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		/**
		 * 检测所有处理程序映射
		 * true:检测所有的 HandlerMapping 类型的bean
		 * false:只要一个beanName为 "handlerMapping" 的HandlerMapping
 		 */
		if (this.detectAllHandlerMappings) {
			// Find all HandlerMappings in the ApplicationContext, including ancestor contexts.
			// 在 ApplicationContext(包括它的祖先容器) 中取出所有类型为ApplicationContext的bean
		...
		// 通过配置文件中的信息得到 handlerMapping
		if (this.handlerMappings == null) {
			//获得 DispatcherServlet.properties 配置文件中那两个默认的实现
			//配置文件在 spring-webmvc项目中的resources\org\springframework\web\servlet\DispatcherServlet.properties
	...
	private void initHandlerAdapters(ApplicationContext context) {
		...
		/**
		 * 检测所有处理程序适配器
		 * true:检测所有的 HandlerAdapter 类型的bean
		 * false:只要一个beanName为 "handlerAdapter" 的 handlerAdapter
		 */
		...
		// 如果都没找到，从配置文件获取处理器适配器 (与处理器映射器的处理逻辑一样)
