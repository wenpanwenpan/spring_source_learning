...
public class AnnotatedBeanDefinitionReader {
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		//把传进来的Class变成spring的bd
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(beanClass);
		//判断这个 beanDefinition 是否要跳过解析
		if (this.conditionEvaluator.shouldSkip(abd.getMetadata())) {
			return;
		}

		abd.setInstanceSupplier(instanceSupplier);
		//解析 beanDefinition ，得到bean的作用域
		ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(abd);
		//解析完成后设置回 beanDefinition 中
		abd.setScope(scopeMetadata.getScopeName());
		//生成一个 bean 名称
		String beanName = (name != null ? name : this.beanNameGenerator.generateBeanName(abd, this.registry));

		//处理类中的通用注解,包括 @Lazy,@Primary,@DependsOn,@Role,@Description
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);

		//除了bean上加的注解外，如果调用这个方法时传过来了，注解信息在这里处理
		if (qualifiers != null) {
		...
		// 比 beanDefinition 功能更加丰富，类似一个 Map < BeanDefinition , beanName >
